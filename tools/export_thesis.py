...
from decision_status import analysis_original_checked, infer_decision
...
CATALOG_DIR = first_existing("catalog", "catalog")
SOURCES = first_existing("sources", "sources")
ANALYSES = first_existing("analyses", "analyses")
EXCERPTS = first_existing("evidence", "evidence")
CATALOG = CATALOG_DIR / ("sources.csv" if (CATALOG_DIR / "sources.csv").exists() else "sources.csv")
SELECTION = CATALOG_DIR / (
    "thesis-selection.csv" if (CATALOG_DIR / "thesis-selection.csv").exists() else "thesis-selection.csv"
)
DEFAULT_OUTPUT = ROOT / ("thesis-package" if (ROOT / "thesis-package").exists() else "thesis-package")
